...
func _process(delta):
	if $LiliacCangur.global_position.x > last_position + 200:
		...
		stalp_1.set_position(Vector2(last_position + 200, 20.6))
		stalp_2.set_position(Vector2(last_position + 200, 265.0))

		stalp_2.global_rotation = stalp_1.global_rotation + PI 
		add_child(stalp_1)
		add_child(stalp_2)
		print("HAI cu tata")
